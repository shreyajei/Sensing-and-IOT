...
// Variable to track flex sensor trigger
let flexSensorTriggered = false;
...
if (!!window.EventSource) {
  ...
  // New event listener for flex sensor
  source.addEventListener('flex_sensor', function (e) {
    console.log("Flex Sensor Value:", e.data);
    
    // Convert data to number and check if it exceeds threshold
    let flexValue = Number(e.data);
    if (flexValue > 400 && !flexSensorTriggered) {
      flexSensorTriggered = true;
      console.log("Chin sensor activated!");
      createFlexSensorPopup();
    }
  }, false);
